...
const imageName = "./upload/earthzeta.jpeg";
...
export const getUploadedMetadataURI = async (): Promise<string> => {
  ...
  try {
    ...
    console.log(
      "IPFS URL:",
      `https://cf-ipfs.com/ipfs/${imageUploadResult.cid}`
    );
    ...
    const metadataUploadResult = await uploadFileToIPFS(
      metadataName,
      metadataContent
    );
    ...
    console.log(
      "IPFS URL:",
      `https://cf-ipfs.com/ipfs/${metadataUploadResult.cid}`
    );
